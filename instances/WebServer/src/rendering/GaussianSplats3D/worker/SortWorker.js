...
import WorkerWasm from "./WorkerWasm"
...
export function createSortWorker(splatCount, useSharedMemory, enableSIMDInSort, integerBasedSort, dynamicMode,
    ...
    const sorter_no_simd_wasm = WorkerWasm.sorter_no_simd_wasm
    const sorter_wasm = WorkerWasm.sorter_wasm
    const sorter_non_shared_wasm = WorkerWasm.sorter_non_shared_wasm
    const sorter_no_simd_non_shared_wasm = WorkerWasm.sorter_no_simd_non_shared_wasm

    // REMOVED and replaced with Base64 encoded string
    // let sourceWasm = SorterWasm;
    let sourceWasm = sorter_wasm
    ...
    if (!enableSIMDInSort && !useSharedMemory) {
        ...
        sourceWasm = sorter_no_simd_wasm
        ...
        if (iOSSemVer && iOSSemVer.major <= 16 && iOSSemVer.minor < 4) {
            ...
            sourceWasm = sorter_no_simd_non_shared_wasm
        }
        ...
        if (iOSSemVer && iOSSemVer.major <= 16 && iOSSemVer.minor < 4) {
            ...
            sourceWasm = sorter_non_shared_wasm
        }
    }
